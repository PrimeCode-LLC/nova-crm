...
# Snapshot cutover stays off until you set this build arg to "true".
# The running web container also needs WORKSPACE_CRM_SNAPSHOT_OFF=true at that
# same cutover. Client and server must match or the browser and API disagree.
ARG NEXT_PUBLIC_WORKSPACE_CRM_SNAPSHOT_OFF
# Client poll ≥60s + dashboard KPI API. Empty keeps legacy 15s / client aggregates.
# Pair with runtime DASHBOARD_KPI_API_V2 / WORKSPACE_CRM_POLL_V2=true.
ARG NEXT_PUBLIC_DASHBOARD_KPI_API_V2
ARG NEXT_PUBLIC_WORKSPACE_CRM_POLL_V2
...
ENV NEXT_PUBLIC_WORKSPACE_CRM_SNAPSHOT_OFF=$NEXT_PUBLIC_WORKSPACE_CRM_SNAPSHOT_OFF
ENV NEXT_PUBLIC_DASHBOARD_KPI_API_V2=$NEXT_PUBLIC_DASHBOARD_KPI_API_V2
ENV NEXT_PUBLIC_WORKSPACE_CRM_POLL_V2=$NEXT_PUBLIC_WORKSPACE_CRM_POLL_V2
