...
function instantlySecretsDoc(orgId: string) {
  const db = getAdminDb();
  if (!db) return null;
  return db
...
export function isInstantlyEncryptionConfigured(): boolean {
  return getSecretsKey() !== null;
}

export async function upsertInstantlyApiKeyServer(input: {
  ...
  const ref = instantlySecretsDoc(input.organizationId);
...
export async function clearInstantlyApiKeyServer(
  organizationId: string,
): Promise<{ ok: true } | { error: string }> {
  const ref = instantlySecretsDoc(organizationId);
  if (!ref) return { error: "Database not configured" };
  await ref.set(
    {
      apiKey: null,
...
export async function getInstantlyApiKeyServer(organizationId: string): Promise<string | null> {
  const key = getSecretsKey();
  if (!key) return null;
  const ref = instantlySecretsDoc(organizationId);
  if (!ref) return null;
...
export async function hasInstantlyApiKeyServer(organizationId: string): Promise<boolean> {
  const ref = instantlySecretsDoc(organizationId);
  if (!ref) return false;
  const snap = await ref.get();
