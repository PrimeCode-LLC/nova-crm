 ...
 * Port of Cloud Functions `processProspectImportChunk` core so BullMQ workers
 * can apply chunks without depending on the Functions package.
 *
 * Firestore remains the chunk/job store. CF trigger is skipped when
 * `QUEUE_IMPORT_CHUNKS_V1=true` (see queue flags).
 */
...
export type ProcessImportChunkInput = {
  organizationId: string;
  jobId: string;
  chunkId: string;
};
...
/**
 * Claim + apply a single import chunk (same semantics as the CF Firestore trigger).
 */
export async function processProspectImportChunkById(
  ...
  const after = chunkSnap.data() as Chunk;
  ...
  const claimed = await firestore.runTransaction(async (transaction) => {
    ...
    const currentChunk = nextChunkSnap.data() as Chunk;
    const limitError =
