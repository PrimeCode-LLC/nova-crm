...
import { mockAccounts, mockContacts, mockLeads, mockDeals } from "@/lib/mock-data";
...
export default async function AccountDetailPage({
  ...
  const leads = mockLeads.filter((l) => l.accountId === account.id);
  const deals = mockDeals.filter((d) => d.accountId === account.id);
                    ...
                    <TableBody>
                      {leads.map((l) => (
                        <TableRow key={l.id}>
                          ...
                          <TableCell className="py-2"><ChannelChip channel={l.channel} /></TableCell>
                          <TableCell className="py-2"><StageBadge stage={l.stage} /></TableCell>
                          <TableCell className="py-2"><UserChip userId={l.ownerId} size="xs" /></TableCell>
                    ...
                    <TableBody>
                      {deals.map((d) => (
                        <TableRow key={d.id}>
                          ...
                          <TableCell className="py-2 text-xs text-muted-foreground">
                            {fmtDate(d.expectedCloseDate, "MMM d")}
                          </TableCell>
                          <TableCell className="py-2"><UserChip userId={d.ownerId} size="xs" /></TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
