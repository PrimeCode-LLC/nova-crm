...
export default async function LeadDetailPage({
  ...
  const profile = getProfileById(lead.profileId);
  const touchpoints = mockTouchpoints.filter((t) => t.leadId === lead.id);
  const timeline = mockTimelineByLead[lead.id] ?? [];
  const notes = mockNotes.filter((n) => n.leadId === lead.id);
  const followups = mockFollowups.filter((f) => f.leadId === lead.id);
          ...
          <div className="p-6 border-r">
            <Tabs defaultValue="overview" className="w-full">
              ...
              <div className="mt-4">
                ...
                <TabsContent value="timeline"><LeadTimeline events={timeline} /></TabsContent>
                <TabsContent value="touchpoints"><LeadTouchpoints touchpoints={touchpoints} /></TabsContent>
                <TabsContent value="notes"><LeadNotes notes={notes} /></TabsContent>
                <TabsContent value="followups"><LeadFollowups followups={followups} /></TabsContent>
              </div>
            </Tabs>
          </div>
          ...
          <aside className="p-6 bg-muted/20 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="pt-0 space-y-2 text-xs text-muted-foreground">
                ...
                {lead.estimatedValue && (
                  <p>
                    Estimated value <span className="font-semibold text-foreground">{fmtCurrency(lead.estimatedValue)}</span>
